...
class FlowdesignController extends AuthController {

	protected function model() {
		if ($this->_obj_model)
			return $this->_obj_model;
		return $this->_obj_model = M('flow');
	}
	...
	public function index() {
		...
		$list = M('flow_process')->where($map)->order('id asc')->select();
		//改变步骤格式，适应 js
		$process_data = array();
		$process_total = 0;
	...
	public function add_process() {
		...
		$process_count = M('flow_process')->where($map)->count();
	...
	public function save_canvas() {
		...
		$one = M('flow')->field('id')->where($map)->find();
		if (!$one) {
			$this->ajaxReturn(array(
				'status' => 0,
				'msg' => '未找到流程数据',
...
//		$this->assign('form_one', $form_one);
	...
	public function get_field_name($field, $field_data) {
		...
		foreach ($field_data as $value) {
			if ($value['smartplugins'] == 'checkboxs' && $value['parse_name'] == $field) {
				$title = $value['title'];
				break;
	...
	public function save_attribute() {
		...
		foreach ($process_condition as $value) {
			$value = intval($value);
			if ($value > 0) {
				$condition = trim($_POST['process_in_set_' . $value], "@smart@");
				$condition = $condition ? explode("@smart@", $condition) : array();
...
//print_r($data);exit;
		...
		$process_model->where($map)->save($data);
		$this->_cleanCacheByFlowId($flow_id);
		//成功返回
		self::return_iframe_ajax(array(
			'status' => 1,
			'msg' => '保存成功',
	...
	private function _cleanCacheByFlowId($flow_id){
		S("FLOW_PROCESS_CACHE_".$flow_id,NULL);
	}
	/**
	 * 选择用户
	 */
