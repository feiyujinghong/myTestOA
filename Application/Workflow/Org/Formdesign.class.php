...
class Formdesign
{
    ...
    */
    public function  parse_form($template,$fields=0)
    {
        ...
        */
        $preg =  "/(\|-<span(((?!<span).)*smartplugins=\"(radios|checkboxs|select)\".*?)>(.*?)<\/span>-\||<(img|input|textarea|select).*?(<\/select>|<\/textarea>|\/>))/s";
        //获取属性  修改为可变 的匹配
        $preg_attr ="/(\w+)=\"(.?|.+?)\"/s";
        //获取单选组复选
        $preg_group ="/<input.*?\/>/s";
        ...
        if($temparr)
        {
            foreach($temparr[0] as $pno=>$plugin)
            {
                ...
                if($tag =='checkboxs') /*复选组  多个字段 */
                {
                    ...
                    $attr_arr_all['content'] = '<span smartplugins="checkboxs" title="'.$attr_arr_all['title'].'">';
                    foreach($parse_group[0] as $value)
                    {
                        ...
                        if($is_new)
                        {
                            $add_fields[$option['name']] = array(
                                'name'=>$option['name'],
                                'smartplugins'=>$attr_arr_all['smartplugins']
                            );
                        }
                ...
                else if($name) 
                {
                    if($tag =='radios') /*单选组  一个字段*/
                    {
                        ...
                        $attr_arr_all['content'] = '<span smartplugins="radios" name="'.$attr_arr_all['name'].'" title="'.$attr_arr_all['title'].'">';
                        foreach($parse_group[0] as $value)
                        {
                            preg_match_all($preg_attr,$value,$parse_attr);
                            $option = array();
                    ...
                    {
                        $attr_arr_all['content'] = $is_new ? str_replace('smartNewField',$name,$plugin) : $plugin;
                    }
    ...
    {
        if($smartplugins=='textarea' or $smartplugins=='listctrl')
        {
            return 'text NOT NULL';
        }
        else if($smartplugins=='checkboxs')
        {
