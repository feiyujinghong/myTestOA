 ...
 */
//namespace Org\Util; 公共使用 import 所以注释
class Input {
    ...
    /**  Cyf 2014-5-14
     +----------------------------------------------------------
     * 获取接口提交的原始数据输入流，通常是api时使用 ， 不接收 multipart/form-data类型的数据
     +----------------------------------------------------------
     * @access public  
     +----------------------------------------------------------
     * @param string $decode 要处理的数据类型
     * 
     * 通常是传入json 的api  处理后返回数组
     +----------------------------------------------------------
     * @return array
     +----------------------------------------------------------
     */
    static public function  rawData($decode = 'json')
    {
     ...
     * 
     * $is_map 默认使用 $map = array()    如果直接写sql  query($sql)  则  $is_map = false   调用   mysql_real_escape_string
     +----------------------------------------------------------
     * @return string
     +----------------------------------------------------------
     */
    static public function forSearch($string,$is_map=true) {
        $string = self::addSlashes($string);//判断gpc  //使搜索  \ 结果不变  正常是 3个\\\  但下面 mysql_real_escape_string 所以 2个\\
        if(!$is_map)//map 默认会处理一次，所以不用处理
    ...
    static function escapeString($str) {
       return mysql_escape_string($str);// mysql_real_escape_string 好像要扩展支持
    }
